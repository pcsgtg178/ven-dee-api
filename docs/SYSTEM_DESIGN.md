...
2. **API Backend**: RESTful API พัฒนาด้วย Express / Node.js (ES Modules).
3. **Database (PostgreSQL - Pure Native Driver / No Prisma ORM)**:
   - ใช้ **PostgreSQL 14+** ร่วมกับไดรเวอร์ `pg` (Node-Postgres Connection Pool) โดยตรง
   - **ไม่ใช้ Prisma ORM** เพื่อความคล่องตัวในการปรับแต่ง DDL, SQL Transactions (`BEGIN ... COMMIT`), และการใช้ Row-level locking (`FOR UPDATE`) เต็มรูปแบบ
   - โครงสร้างตาราง (DDL) ถูกจัดการผ่าน `scripts/schema.sql` และสคริปต์ JavaScript สำหรับการ Migrate/Seed
...
## 7. คู่มือการจัดการฐานข้อมูล PostgreSQL (PostgreSQL Database & Migration Scripts Guide)

ระบบ VenDee API ใช้ **PostgreSQL แบบ Native (Pure SQL + node-postgres `pg` Driver)** โดยไม่พึ่งพา Prisma ORM เพื่อความเรียบง่าย ประสิทธิภาพสูงสุด และความแม่นยำในการควบคุม Transaction

### 7.1 รายการสคริปต์ในโฟลเดอร์ `scripts/`
| ไฟล์ | คำอธิบาย |
| :--- | :--- |
| `scripts/schema.sql` | ไฟล์ PostgreSQL DDL ดั้งเดิม สำหรับสร้าง Extensions, ตารางทั้งหมด (`shifts`, `shift_swap_transactions`, `swap_trail_nodes`, `customers`, `customer_services`), Foreign Keys และ B-Tree Indexes |
| `scripts/seed.sql` | ไฟล์ SQL ข้อมูลเริ่มต้น (Mock Data) สำหรับจำลองการใช้งาน (ข้อมูลลูกค้า, เวรโรงพยาบาล, นัดหมายหัตถการ) |
| `scripts/init-db.js` | สคริปต์ Node.js เชื่อมต่อ Pool และรัน `schema.sql` |
| `scripts/seed.js` | สคริปต์ Node.js เชื่อมต่อ Pool และรัน `seed.sql` |
| `scripts/reset-db.js` | สคริปต์ Node.js ล้างตารางทั้งหมด (DROP TABLES CASCADE) แล้วสร้าง Schema ใหม่พร้อมลง Seed Data |

### 7.2 คำสั่งจัดการฐานข้อมูลผ่าน npm scripts
```bash
# 1. รัน Migration สร้างโครงสร้างตารางใน PostgreSQL
npm run db:init

# 2. นำเข้าข้อมูลตัวอย่าง (Seed Data)
npm run db:seed

# 3. ล้างฐานข้อมูลและเริ่มต้นใหม่ทั้งหมด (Reset + Re-seed)
npm run db:reset
```

### 7.3 ตัวแปรสภาพแวดล้อม (Environment Variables)
ตั้งค่าในไฟล์ `.env`:
```env
# รูปแบบ Connection String ของ PostgreSQL
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/vendee_db
