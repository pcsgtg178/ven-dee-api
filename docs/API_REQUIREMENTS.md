...
### 1.5 ฐานข้อมูลและเทคโนโลยีจัดเก็บข้อมูล (Database Technology - Pure PostgreSQL)
- **Database Engine:** **PostgreSQL 14+** (Native Driver: `pg` / Node-Postgres Connection Pool)
- **ORM / Query Strategy:** **ไม่ใช้ Prisma ORM** (Pure SQL DDL & Native SQL Queries) เพื่อให้การควบคุม ACID Transaction, Isolation Level และ Row-Level Locking (`FOR UPDATE`) ทำงานได้อย่างมีประสิทธิภาพและยืดหยุ่นสูงสุด
- **Database DDL & Migration Scripts:** จัดเก็บอยู่ที่ `scripts/schema.sql` และรันผ่านคำสั่ง `npm run db:init`
- **Seed Data Scripts:** จัดเก็บอยู่ที่ `scripts/seed.sql` และรันผ่านคำสั่ง `npm run db:seed`
...
```json
{
  ...
  "error": {
    ...
    "message": "ไม่สามารถแลกมารับเวรบ่าย (16:00 - 24:00) ในวันที่ 2026-09-24 ได้ เนื่องจากมีนัดหมายบริการ \"คุณยายสมศรี สุขเกษม\" เวลา 16:30 น. อยู่แล้ว"
  }
}
```
...
1. [ ] **Database Migration (Pure PostgreSQL - No Prisma):** รัน `npm run db:init` เพื่อสร้าง Tables ตาม ERD (`shifts`, `shift_swap_transactions`, `swap_trail_nodes`, `customers`, `customer_services`) ผ่าน `scripts/schema.sql`
...
3. [ ] **Transaction Guard:** ฟังก์ชันแลกเวร (`/shifts/:id/swap`) และยกเลิกการแลก (`/shifts/:id/undo-swap`) ต้องอยู่ภายใต้ SQL Transaction (`client.query('BEGIN') ... COMMIT`)
